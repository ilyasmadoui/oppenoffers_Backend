    ...
    insertSelectedSupplier: async (data) => {
        try {
            ...
            await pool
                ...
                .input("aEmail", sql.NVARCHAR(50), Email)
                .input("adminID", sql.UniqueIdentifier, adminID)
                .execute("insertFournisseurPotential");

            return { success: true };
        } catch (error) {
            // Catch the specific numbers from the THROW command in SQL
            if (error.number === 51004) {
                return { success: false, code: 1004, message: "Le numéro de téléphone est déjà utilisé." };
            } 
            if (error.number === 51005) {
                return { success: false, code: 1005, message: "L'email est déjà utilisé." };
            }

            console.error("Service error (insertSelectedSupplier):", error);
            return { success: false, code: 5000, message: "Erreur serveur lors de l'ajout." };
        }
    },
}