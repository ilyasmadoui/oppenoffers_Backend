...
export const addComissionMemeber = async (
    ...
    try {
        console.log("Received: ", Nom, Prenom, Fonction, Email, Role, adminId);
        ...
        // Check output parameter or return value
        const insertResult = result.returnValue;

        console.log("Stored procedure return value:", insertResult);

        switch (insertResult) {
            case 0:
                return { success: true, code: 0, message: 'Succès.' };
            case 1001:
                return { success: false, code: 1001, message: "Rôle invalide." };
            case 1002:
                return { success: false, code: 1002, message: "Email existe déjà." };
            case 1004:
                return { success: false, code: 1004, message: "ID Administrateur introuvable." };
            case 1005:
                return { success: false, code: 1005, message: "Il existe déjà un Président." };
            case 1006:
                return { success: false, code: 1006, message: "Il existe déjà un Secrétaire." };
            case 1000:
            default:
                // Log the actual SQL error for debugging
                console.error("SQL Error details:", result);
                return { success: false, code: 1000, message: "Erreur lors de l'ajout." };
        }
    } catch (error) {
        console.error("Commission Member services error: ", error);
...
export const getAllCommissionMembers = async (adminId) => {
    try {
        console.log("getAllCommissionMembers service Recieved : ", adminId)
        const pool = await poolPromise;
        const result = await pool.request()
            .input('adminId', sql.UniqueIdentifier, adminId)
