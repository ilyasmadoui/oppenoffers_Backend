...
module.exports = {
    ...
    deleteOperationSQL: async (NumOperation) => {
        try {
            ...
            if(resultCode === 0) {
                return { code:0, message:"Suppression réussie"};
            }
            if(resultCode === 1003) {
                return { code:1003, message:"Opération non trouvée"};
            }
            if (resultCode === 1000) {
                return { code: 1000, message: "Opération liée à des fournisseurs" };
            }
            return { code: resultCode };
        } catch (error) {
    ...
    manageArchiveOperationMysql: async(id) => {
        try{
            const [rows] = await db.execute('CALL manageActivateOperation(?)', [id]);
            const code = rows[0][0].Result;
            ...
            console.error("Operation service error (manageArchiveOperationMysql):", error);
            return{
                success: false,
                code: 5000,
