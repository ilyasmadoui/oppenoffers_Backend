...
module.exports = {
    ...
    createRetrait: async (req, res) => {
        try {
            ...
            if (!data.SupplierID || !data.OperationID || !data.NumeroRetrait || !data.AnnonceID) {
    ...
    getRetraitsWithSpecs: async (req, res) => {
        try {
            const { annonceID } = req.query; // on récupère l'AnnonceID depuis la query

            if (!annonceID) {
                return res.status(400).json({
                    success: false,
                    message: "AnnonceID est requis"
                });
            }

            // Utiliser la fonction du service correctement
            const result = await retraitService.getRetraitsWithSpecsSQL(annonceID);
            ...
            console.error('Controller error (getRetraitsWithSpecs):', error);
            return res.status(500).json({
                success: false,
