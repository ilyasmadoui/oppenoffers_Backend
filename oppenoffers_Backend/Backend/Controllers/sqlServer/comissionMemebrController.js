...
export const addComissionMemberController = async (req, res) => {
    const { Nom, Prenom, Fonction, Email, Role, adminId } = req.body;
    
    console.log("Controller received:", { Nom, Prenom, Fonction, Email, Role, adminId });
    
    if (!Nom || !Prenom || !Email || !Role || !adminId) {
        return res.status(400).json({ 
            success: false, 
            message: "Nom, Prenom, Email, Role, adminId are required." 
        });
    }
    
    try {
        const result = await comissionMemeberServices.addComissionMemeber(
            Nom, Prenom, Fonction, Email, Role, adminId
        );
